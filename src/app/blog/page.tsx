    ...
    <div className="section">
      <div className="container">
        <div style={{ marginBottom: '2rem' }}>
          <p className="label" style={{ marginBottom: '0.5rem' }}>Blog</p>
          <h1 style={{ fontSize: '2rem', fontWeight: 800, marginBottom: '0.5rem' }}>Blog Posts</h1>
          <p className="text-secondary">{posts.length} expert articles to help you choose the right gear</p>
        </div>

        {/* Category filters */}
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
          {activeCats.map(cat => (
            <span key={cat.slug} className="badge" style={{ cursor: 'pointer' }}>{cat.name} <span style={{ opacity: 0.6 }}>({cat.count})</span></span>
          ))}
        </div>
          ...
          {sortedPosts.map((post) => (
            <Link key={post.id} href={`/blog/${post.slug}`} className="card" style={{ display: 'block', overflow: 'hidden' }}>
              <div style={{ padding: '1.5rem' }}>
                ...
                <h2 style={{ fontSize: '1.125rem', fontWeight: 700, marginBottom: '0.5rem', lineHeight: 1.3 }}>{post.title}</h2>
                <p className="text-secondary" style={{ fontSize: '0.875rem', marginBottom: '1rem', lineHeight: 1.6 }}>{post.excerpt}</p>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span className="text-accent" style={{ fontSize: '0.75rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Read More →</span>
                </div>
              </div>
            </Link>
          ))}
