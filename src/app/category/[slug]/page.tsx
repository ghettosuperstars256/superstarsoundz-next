...
const categoryIcons: Record<string, string> = {
  ...
  'headphones-and-iems': 'Headphones & IEMs',
  'keyboards-and-synthesizers': 'Keyboards & Synths',
...
export default async function CategoryPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    url: `https://superstarsoundz.com/category/${slug}`,
  };

  const iconLabel = categoryIcons[cat.slug] || cat.name;
  ...
  return (
    <>
      ...
      <div style={{
        background: 'linear-gradient(135deg, #0e0e12 0%, #08080a 100%)',
        ...
        <div style={{
          ...
          fontSize: '0.875rem', opacity: 0.15, pointerEvents: 'none', fontWeight: 700, letterSpacing: '0.05em',
        }}>{iconLabel}</div>
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.75rem' }}>
            <span style={{ fontSize: '2.5rem' }}>{iconLabel.charAt(0)}</span>
            <div>
      ...
      {/* Category pills */}
      <div style={{ borderBottom: '1px solid #1e1e26', background: '#08080a' }}>
        <div className="container" style={{ padding: '1rem 1.5rem' }}>
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', overflowX: 'auto' }}>
            ...
            {productCategories.filter(c => c.slug !== slug).map(c => (
            ...
            {filtered.map(product => (
              <Link key={product.id} href={`/gear/${product.slug}`} className="card product-card" style={{ position: 'relative', overflow: 'visible' }}>
                ...
                <div className="product-card-image" style={{ borderRadius: '8px 8px 0 0' }}>
                  ...
                  <p className="text-accent" style={{ fontSize: '0.6875rem', fontWeight: 600, marginBottom: '0.375rem', textTransform: 'none' }}>
