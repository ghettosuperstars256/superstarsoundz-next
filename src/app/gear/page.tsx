...
function GearContent() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case 'name':
        return [...filtered].sort((a, b) => a.short_name.localeCompare(b.short_name));
      default:
        return filtered;
    }
  }, [activeCat, sortBy]);
  ...
  return (
    <>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          <button
            ...
            style={{
              ...
              borderRadius: '4px',
              border: '1px solid',
              borderColor: activeCat === 'all' ? '#D4A843' : '#222222',
              background: activeCat === 'all' ? 'rgba(212, 168, 67, 0.1)' : 'transparent',
              color: activeCat === 'all' ? '#D4A843' : '#888888',
              fontSize: '0.75rem',
              fontWeight: 600,
          ...
          {productCategories.map(cat => (
            <button
              ...
              style={{
                ...
                borderRadius: '4px',
                border: '1px solid',
                borderColor: activeCat === cat.slug ? '#D4A843' : '#222222',
                background: activeCat === cat.slug ? 'rgba(212, 168, 67, 0.1)' : 'transparent',
                color: activeCat === cat.slug ? '#D4A843' : '#888888',
                fontSize: '0.75rem',
                fontWeight: 600,
                textTransform: 'uppercase',
                letterSpacing: '0.05em',
        ...
        {filteredProducts.map(product => (
          <Link key={product.id} href={`/gear/${product.slug}`} className="card product-card">
            ...
            <div style={{ padding: '1rem' }}>
              ...
              <p style={{ fontSize: '0.75rem', color: '#555555', marginBottom: '0.75rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {product.short_description}
              </p>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '1rem', fontWeight: 700, color: '#D4A843' }}>${product.price}</span>
                <span style={{ fontSize: '0.6875rem', color: '#D4A843', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  View Deal →
                </span>
              </div>
            </div>
      ...
      {filteredProducts.length === 0 && (
        <div style={{ textAlign: 'center', padding: '4rem 0', color: '#555555' }}>
    ...
    <div className="section">
      <div className="container">
        <div style={{ marginBottom: '2rem' }}>
          ...
          <p style={{ color: '#888888' }}>23 products</p>
        </div>

        <Suspense fallback={<div style={{ padding: '2rem', textAlign: 'center', color: '#555555' }}>Loading...</div>}>
          <GearContent />
        </Suspense>
      </div>
    </div>
